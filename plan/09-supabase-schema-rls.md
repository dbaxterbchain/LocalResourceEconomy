...
This document describes the initial Supabase schema and row-level security rules. It is a planning artifact and can be converted into migration SQL later.
...
create table surveys (
  ...
  contact_info_mode text not null default 'optional', -- optional|required
  contact_info_placement text not null default 'end', -- start|end
...
## Edit response item RPC (audit logged)
```sql
...
## Edit response contact info RPC (audit logged)
```sql
...
## Public survey bundle RPC (slug scoped, open only)
```sql
...
## Staff preview bundle RPC (slug scoped)
```sql
...
-- Public read for survey definitions and links
...
-- Public insert responses only when link is open and within open/close window
...
## Notes
- Public read policies require `app.public_slug` to be set for the current request. Use an RPC such as `get_public_survey_bundle(slug)` to set the config and return data.
- If you prefer JWT-based scoping, issue a short-lived token with a `public_slug` claim and read it via `auth.jwt()`.
- Audit-only updates are enforced by requiring `app.audit_reason` to be set in update policies. RPCs set this value before applying updates.
- Public bundle only returns open links; use `get_staff_survey_bundle` for previewing closed or draft links.
